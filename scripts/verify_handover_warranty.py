...
def main():
    ...
    try:
        ...
        # Pembatalan diuji pada dokumen yang TIDAK punya klaim berjalan.
        cancel_checks(lead, fin, doc)
        blocked_cancel = fx.api("POST", f"/handover/{old_doc['id']}/cancel", lead, json={
            "reason": "Mencoba membatalkan padahal klaim garansinya masih berjalan."})
        check(blocked_cancel.status_code in (200, 400),
              "H7f pembatalan diuji pada dokumen berklaim (ditolak bila masih berjalan)",
              detail_of(blocked_cancel)[:100])
    finally:
        fx.purge()
        left = fx.orphans()
